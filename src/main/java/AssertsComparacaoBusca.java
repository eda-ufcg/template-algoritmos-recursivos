public class AssertsComparacaoBusca {
    ...
    public static void testLimitesIterativa() {
        assert comp.buscaBinariaIterativa(v, 1) == 0;
        assert comp.buscaBinariaIterativa(v, 89) == v.length - 1;
    }

    public static void testLimitesIterativaNaoEncontrou() {
        assert comp.buscaBinariaIterativa(v, -9000) == -1;
        assert comp.buscaBinariaIterativa(v, 90000) == -1;
    }

    public static void testLimitesRecursiva() {
        assert comp.buscaBinariaRecursiva(v, 0, v.length - 1, 1) == 0;
        assert comp.buscaBinariaRecursiva(v, 0, v.length - 1, 89) == v.length - 1;
    }

    public static void testLimitesRecursivaNaoEncontrou() {
        assert comp.buscaBinariaRecursiva(v, 0, v.length - 1, -9000) == -1;
        assert comp.buscaBinariaRecursiva(v, 0, v.length - 1, 90000) == -1;
    }
}
